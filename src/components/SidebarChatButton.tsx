...
import IconBxChat from "./icons/iconBxChat";
import IconTrash3 from "./icons/iconTrash";
import IconEdit3 from "./icons/iconEdit3";
import IconClose from "./icons/inconClose";
...
export const SidebarChatButton = ({ chatItem, active, onClick, onDelete, onEdit }: Props) => {
    ...
    return (
        <div onClick={handleClickButton} className={`flex items-center roundede-md p-3 text-sm cursor-pointer hover:bg-gray-500/10 ${active ? 'bg-gray-500/20' : 'bg-transparent'}`}>

            <div className="mr-3">
                {!deleting && <IconBxChat width={16} height={16} />}
